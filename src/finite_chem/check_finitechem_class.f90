...
contains
   ...
   subroutine react(this, dt)
      ...
      real(WP) :: mytend, mytstart, mytstop, mydt
      integer :: myitask,myistate
      real(WP), dimension(22) :: rstats
      integer, dimension(31) :: istats


      ! Highjack NGA2 routines to calculate homogeneous reactor solution
      k = this%cfg%kmino_
      j = this%cfg%jmino_
      i = this%cfg%imino_
      ...
      mytend = 1e-3
      mydt = 1e-6
      myitask = 1; myistate = 1; mytstart = 0.0_WP; mytstop = mydt
      opts = set_opts(method_flag=22, mxstep=500000, abserr=atol, relerr=rtol, tcrit=mytstop)

      sol(1:nspec1) = 0.0_WP
   
      !sol(sHMN) = 1.0810831e-02_WP
      !sol(sXC12H26) = 5.2061624e-02_WP
      !sol(sN2) = 7.1876980e-01_WP
      !sol(sO2) = 2.1835775e-01_WP
      !sol(nspec1) = 990

      sol(sH2) = 2.8520971e-02_WP
      sol(sN2) = 7.4511713e-01_WP
      sol(sO2) = 2.2636190e-01_WP
      sol(nspec1) = 1200

      print*, '# time T H2'
      print*, mytstart, sol(nspec1), sol(sH2)
      do while (mytstart .lt. mytend)
         call dvode_f90(fc_reaction_compute_rhs, nspec1, sol, mytstart, mytstop, myitask, myistate, opts)
         mytstart = mytstop
         mytstop = mytstop + mydt
         print*, mytstop, sol(nspec1), sol(sH2)
      end do
...
!    !                     end if
   ...
   contains
      ...
      subroutine fc_reaction_compute_rhs(n_, t_, sol, rhs)
         ...
         ! Calculate concentration-based reaction rates
         call fcmech_get_wdot(this%Pthermo, sol(nspec+1), sol(1:nspec), wdot)

         ! Convert into mass-based reaction rates
