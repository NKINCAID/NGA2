...
module multimatrix_class
   ...
   interface multimatrix
      procedure multimatrix_from_file
...
contains
   ...
   function multimatrix_from_file(fdata,name) result(self)
      ...
      integer :: ierr,imatrix,n_row,n_col
      type(MPI_File) :: ifile
      ...
      allocate(self%matnames(1:self%nmatrix))
      allocate(self%matrices(1:self%nmatrix))

      ! Read the matrices
      do imatrix=1,self%nmatrix
         call MPI_FILE_READ_ALL(ifile,self%matnames(imatrix),str_medium,MPI_CHARACTER,status,ierr)
   ...
   end function multimatrix_from_file
