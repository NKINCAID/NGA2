...
module pdf_class
    ...
    integer, dimension(part_nblock) :: part_lblock = [8 + nspec, 4]
...
contains
  ...
  allocate (self%npincell_inject(self%cfg%imino_:self%cfg%imaxo_, self%cfg%jmino_:self%cfg%jmaxo_, self%cfg%kmino_:self%cfg%kmaxo_)); self%npincell_inject = 0.0_WP
    ...
    subroutine transport_step(this, dt, U, V, W, rho, visc, diff)
      ...
      real(WP), dimension(this%cfg%imino_:this%cfg%imaxo_, this%cfg%jmino_:this%cfg%jmaxo_, this%cfg%kmino_:this%cfg%kmaxo_), intent(inout) :: diff  !< Needs to be (imino_:imaxo_,jmino_:jmaxo_,kmino_:kmaxo_)
        ...
        do i = 1, this%np_
            ...
            ! ! Handle particles inside walls
            ! if (this%cfg%VF(this%p(i)%ind(1), this%p(i)%ind(2), this%p(i)%ind(3)) .eq. 0.0_WP) then
            !   this%p(i)%multi = 0
            ! end if
        end do
      ...
      real(WP), dimension(this%cfg%imino_:this%cfg%imaxo_, this%cfg%jmino_:this%cfg%jmaxo_, this%cfg%kmino_:this%cfg%kmaxo_), intent(in) :: W     !< Needs to be (imino_:imaxo_,jmino_:jmaxo_,kmino_:kmaxo_)
        ...
        do n = 1, itr%n_
            ...
            loop1: do while (vadded .lt. vgoal)
                ...
                if (vgoal - vadded .lt. 0.01_WP*Vp) exit loop1
                ...
                if (vgoal - vadded .lt. Vp) then
                    ...
                    if (this%cfg%nz .eq. 1) then
                        this%p(count)%pos = [random_uniform(this%cfg%x(i), this%cfg%x(i) + U(i, j, k)*dt),&
    ...
    subroutine control(this)
        ...
        logging1: block
            ...
            character(len=str_long) :: message
            if (this%cfg%amRoot) then
                write (message, '("In the control step...")')
                if (verbose .gt. 1) write (output_unit, '(a)') trim(message)
                if (verbose .gt. 0) call log(message)
            end if
        end block logging1
        ...
        logging2: block
            ...
            character(len=str_long) :: message
            if (this%cfg%amRoot) then
                write (message, '("After main control loop...")')
                if (verbose .gt. 1) write (output_unit, '(a)') trim(message)
                if (verbose .gt. 0) call log(message)
            end if
        end block logging2
        ...
        logging: block
            ...
            character(len=str_long) :: message
            if (this%cfg%amRoot) then
                write (message, '("Particle control step...")')
                if (verbose .gt. 1) write (output_unit, '(a)') trim(message)
                if (verbose .gt. 0) call log(message)
            end if
        end block logging
    ...
    subroutine react(this, dt)
        ...
        if (.not. use_scheduler .or. this%cfg%nproc .eq. 1) then
            ...
            do i = 1, this%np_
                ...
                sol(1:nspec) = this%p(i)%Yi
                sol(nspec1) = this%p(i)%T
                ...
                this%p(i)%Yi = sol(1:nspec)
                this%p(i)%T = sol(nspec1)
  ...
  if (abs(this%p(i)%h-myh)/myh.gt.0.01_WP) print*,'===== P1 - ENTHALPY LIKELY WRONG!!!',i,this%cfg%rank,' - H = ',this%p(i)%h,myh,(this%p(i)%h-myh)/myh 
        ...
        !         sol(1:nspec) = pmc(iDI(ipmc_))%Yi
        !         sol(1:nspec) = sol(1:nspec)/sum(sol(1:nspec))
        !         sol(nspec + 1) = pmc(iDI(ipmc_))%T
        !         ! Buffer composition to send out to other processors
        !         ndata = ndata + 1
        !         bufferS((ndata - 1)*nspec1 + 1:ndata*nspec1) = sol
        ...
        !    call MPI_recv(bufferR(1:nwhere(iworker)*nspec1), nwhere(iworker)*nspec1, MPI_REAL_WP, iworker - 1, itag_data, comm, status, ierr)
        !         ! Store each bufferR compo at correct location in pmc array
        !         do i = 1, nwhere(iworker)
        !           myi = iwhere(iworker, i)
        !           pmc(myi)%Yi = bufferR((i - 1)*nspec1 + 1:i*nspec1 - 1)
        !           pmc(myi)%T = bufferR(i*nspec1)
        !           ! Update other particle variables
        !           ! Enthalpy - Should be constant since reaction step is at constant pressure
        !           myh = pmc(myi)%h
        ...
        !         call MPI_send(bufferS(1:nwhere(iworker)*nspec1), ndata*nspec1, MPI_REAL_WP, iworker - 1, itag_data, comm, ierr)
        ...
        !         ! Send them to head
        !         call MPI_send(bufferR(1:ndata*nspec1), ndata*nspec1, MPI_REAL_WP, ihead - 1, itag_data, comm, ierr)
        ...
        !         call MPI_recv(bufferR(1:ndata*nspec1), ndata*nspec1, MPI_REAL_WP, ihead - 1, itag_data, comm, status, ierr)

        !         ! Do the work
        !         do i = 1, ndata
        !           call pdf_reaction_compute_sol(bufferR((i - 1)*nspec1 + 1:i*nspec1), Pthermo, dt)
        !         end do
    ...
    contains
        ...
        subroutine pdf_reaction_compute_sol(sol, mypressure, dt)
            ...
            ! ------ Direct integration ------ !
            ! -------------------------------- !
            call pdf_reaction_compute_rhs(nspec1, 0.0_WP, solcheck, dsol)
           ...
           call dvode_f90(pdf_reaction_compute_rhs, nspec1, sol, tstart, tstop, itask, istate, opts, j_fcn=pdf_reaction_compute_jac)

                else

                    opts = set_opts(method_flag=22, mxstep=500000, abserr=atol, relerr=rtol, tcrit=tstop)
                    call dvode_f90(pdf_reaction_compute_rhs, nspec1, sol, tstart, tstop, itask, istate, opts)
        ...
        subroutine pdf_reaction_compute_rhs(n_, t_, sol, rhs)
            ...
            ! Get W of mixture
            call pdf_reaction_Wmix(sol(1:nspec), 'Y', Wmix)

            ! Get Cp of mixture and update hsp
            call pdf_reaction_Cpmix(sol(1:nspec), sol(nspec1), Cp_mix)
        ...
        subroutine pdf_reaction_Wmix(scalar, type, Wmix)
            ...
            character :: type
            real(WP) :: Wmix
            real(WP), dimension(nspec) :: scalar, scalar_t
        ...
        subroutine pdf_reaction_compute_jac(n_, t_, sol, ml, mu, jac, nrpd)
            ...
            ! call pdf_reaction_getjacobian(sol(1:nspec), sol(nspec + 1), M, Cdot, K, RHOmix, Cp_mix, Wmix, jac)
        ...
        !   real(WP), dimension(nspec) :: myY
        !   real(WP), dimension(nspec + npTB) :: myCM
        !   ! Temperature
        !   real(WP) :: myT
        !   ! Cdot
        !   real(WP), dimension(nspec) :: myCdot, myYdot
        !   ! Reaction coefficients
        !   real(WP), dimension(nreac + nreac_reverse) :: myK
        !   ! Mixture density, heat capacity, molar mass
        !   real(WP) :: myrho, myCpmix, myWmix
        !   ! Jacobian
        !   real(WP), dimension(nspec1, nspec1), intent(out) :: jac

        !   ! Working arrays
        !   real(WP) :: myTdot, FC, dlnFCdT, dGdT, dlnFdT, k0overkInf
        !   real(WP), dimension(nspec1, nspec1) :: jacC
        !   real(WP), dimension(nspec1, nspec1) :: dCdY
        !   integer :: i, myi, j, myj, jj, myjj, k, ireac, iM
        !   real(WP), dimension(nreactants_max) :: dW_dC
        !   real(WP), dimension(nspec, nspec) :: dCdot_dC
        !   real(WP), dimension(nspec) :: dCdot_dT
        !   real(WP), dimension(nspec) :: dCpsdT
        !   real(WP), dimension(nreac + nreac_reverse) :: dk_dT
        !   real(WP) :: oneoverT, oneoverRT, lnT, Mval, Pr, dW_dT
        !   real(WP), dimension(npRL + 1:nreac + nreac_reverse) :: k0, kinf, dk0_dT, dkinf_dT
        ...
        ! ! Shorthand notations
        ! oneoverT = 1.0_WP/myT
        ! oneoverRT = oneoverT/Rcst
        ! lnT = log(myT)

        ! ! ---------------------------------------------------------- !
        ! ! Temperature source term
        ! myYdot = W_sp*myCdot/myrho
        ...
        ! do i = 1, nspec
        !   do j = 1, nspec
        !     dCdY(i, j) = -myCM(i)*myWmix/W_sp(j)
        !   end do
        !   dCdY(i, i) = dCdY(i, i) + myrho/W_sp(i)
        ! end do
        ! dCdY(nspec1, nspec1) = 1.0_WP
        ...
        !   !kInf(npRL+i) = Acoeff(nreac + nreac_reverse+i) * exp(ncoeff(nreac + nreac_reverse+i)*lnT - Eact(nreac + nreac_reverse+i)*oneoverRT)
        !   kInf(npRL + i) = Acoeff1(nreac + nreac_reverse + i)*exp(Acoeff2(nreac + nreac_reverse + i)*log(10.0_WP) + ncoeff(nreac + nreac_reverse + i)*lnT - Eact(nreac + nreac_reverse + i)*oneoverRT)
        !   dkinf_dT(npRL + i) = oneoverT*(ncoeff(nreac + nreac_reverse + i) + Eact(nreac + nreac_reverse + i)*oneoverRT)*kInf(npRL + i)
        ! end do

        ! ! Get dk_dT for pressure-dependent reactions, with k = kinf*(Pr/1+Pr)*F
        ! do k = npRL + 1, nreac + nreac_reverse
        ...
        ! jac1: do ireac = 1, nreac + nreac_reverse
        ...
        !   ! dYdot_i/dC_j
        !   do j = 1, nspec
        !     jac(i, j) = W_sp(i)*dCdot_dC(i, j)/myrho - myYdot(i)*W_sp(j)/myrho
        !   end do
        !   ! dYdot_i/dT
        !   jac(i, nspec1) = W_sp(i)*(dCdot_dT(i)/myrho - myCdot(i)/(myrho*myT))
        ! end do

        ! ! Accounting for the change of variables (T terms work better in Y space)
        ! jac(1:nspec, 1:nspec) = matmul(jac(1:nspec, 1:nspec), dCdY(1:nspec, 1:nspec))

        ! ! ==================== !
        ! ! dTdot/dY_j
        ! ! ==================== !

        ! jac(nspec1, 1:nspec) = Cpsp*myTdot
        ! do j = 1, nspec
        !   jac(nspec1, j) = jac(nspec1, j) + sum(hsp*jac(1:nspec, j))
        ! end do
        ! jac(nspec1, :) = -jac(nspec1, :)/myCpmix
        ...
        ! jac(nspec1, nspec1) = -(sum(dCpsdT*myY)*myTdot + sum(Cpsp*myCdot) + sum(hsp*jac(1:nspec, nspec1)))/myCpmix
    ...
    real(WP), dimension(this%cfg%imino_:this%cfg%imaxo_, this%cfg%jmino_:this%cfg%jmaxo_, this%cfg%kmino_:this%cfg%kmaxo_), intent(inout) :: delta  !< Needs to be (imino_:imaxo_,jmino_:jmaxo_,kmino_:kmaxo_)
        ...
        logging: block
            ...
            character(len=str_long) :: message
            if (this%cfg%amRoot) then
                write (message, '("Particle mixing step...")')
                if (verbose .gt. 1) write (output_unit, '(a)') trim(message)
                if (verbose .gt. 0) call log(message)
            end if
        end block logging
    ...
    subroutine evap(this, dt, ms, Ys, hs)
        use messager, only: die
        implicit none

        class(pdf), intent(inout) :: this
        real(WP), intent(inout) :: dt  !< Timestep size over which to advance
    real(WP), dimension(this%cfg%imino_:this%cfg%imaxo_, this%cfg%jmino_:this%cfg%jmaxo_, this%cfg%kmino_:this%cfg%kmaxo_), intent(inout) :: ms     !< Needs to be (imino_:imaxo_,jmino_:jmaxo_,kmino_:kmaxo_)
    real(WP), dimension(this%cfg%imino_:this%cfg%imaxo_, this%cfg%jmino_:this%cfg%jmaxo_, this%cfg%kmino_:this%cfg%kmaxo_), intent(inout) :: hs     !< Needs to be (imino_:imaxo_,jmino_:jmaxo_,kmino_:kmaxo_)
    real(WP), dimension(this%cfg%imino_:this%cfg%imaxo_, this%cfg%jmino_:this%cfg%jmaxo_, this%cfg%kmino_:this%cfg%kmaxo_, nspec), intent(inout) :: ys 
        integer :: i, j, k, ip, ii

        real(WP) :: Wmix, sv, m_old
        ! Parameters
        ...
        do ip = 1, this%np_
            ...
            ! Advance compositions
            m_old = this%p(ip)%m
            this%p(ip)%m = this%p(ip)%m + this%p(ip)%m*ms(i, j, k)*this%SV(i, j, k)*dt/this%multi(i, j, k)
this%p(ip)%Yi = this%p(ip)%Yi + this%p(ip)%m*ms(i,j,k)*this%SV(i, j, k)*dt/(m_old*this%multi(i, j, k))*(Ys(i,j,k,:) - this%p(ip)%Yi)
 this%p(ip)%H = this%p(ip)%H + this%p(ip)%m*ms(i, j, k)*this%SV(i, j, k)*dt/(m_old*this%multi(i, j, k))*(Hs(i, j, k) - this%p(ip)%H)
    ...
    subroutine get_eulerian_fields(this)
        ...
        do i = 1, this%np_
            ...
            this%Yi(myi, myj, myk, 1:nspec) = this%Yi(myi, myj, myk, 1:nspec) + this%p(i)%multi*this%p(i)%m*this%p(i)%Yi
        ...
        do k = this%cfg%kmino_, this%cfg%kmaxo_
            do j = this%cfg%jmino_, this%cfg%jmaxo_
                do i = this%cfg%imino_, this%cfg%imaxo_
                    ...
                    this%Yi(i, j, k, 1:nspec) = this%Yi(i, j, k, 1:nspec)/this%multi(i, j, k)
        ...
        subroutine update_boundary(this)
            ...
            if (this%cfg%iproc .eq. 1) then
                do i = this%cfg%imino_, this%cfg%imin_ - 1
                    ...
                    this%Yi(i, :, :, 1:nspec) = this%Yi(this%cfg%imin_, :, :, 1:nspec)
            ...
            if (this%cfg%iproc .eq. this%cfg%npx) then
                do i = this%cfg%imax_ + 1, this%cfg%imaxo_
                    ...
                    this%Yi(i, :, :, 1:nspec) = this%Yi(this%cfg%imax_, :, :, 1:nspec)
            ...
            if (this%cfg%jproc .eq. 1) then
                do j = this%cfg%jmino_, this%cfg%jmin_ - 1
                    ...
                    this%Yi(:, j, :, 1:nspec) = this%Yi(:, this%cfg%jmin_, :, 1:nspec)
            ...
            if (this%cfg%jproc .eq. this%cfg%npy) then
                do j = this%cfg%jmax_ + 1, this%cfg%jmaxo_
                    ...
                    this%Yi(:, j, :, 1:nspec) = this%Yi(:, this%cfg%jmax_, :, 1:nspec)
            ...
            if (this%cfg%kproc .eq. 1) then
                do k = this%cfg%kmino_, this%cfg%kmin_ - 1
                    ...
                    this%Yi(:, :, k, 1:nspec) = this%Yi(:, :, this%cfg%kmin_, 1:nspec)
            ...
            if (this%cfg%kproc .eq. this%cfg%npz) then
                do k = this%cfg%kmax_ + 1, this%cfg%kmaxo_
                    ...
                    this%Yi(:, :, k, 1:nspec) = this%Yi(:, :, this%cfg%kmax_, 1:nspec)
    ...
    subroutine get_eulerian_fields_mid(this)
        ...
        logging: block
            ...
            character(len=str_long) :: message
            if (this%cfg%amRoot) then
                write (message, '("Particle get_eulerian_fields_mid...")')
                if (verbose .gt. 1) write (output_unit, '(a)') trim(message)
                if (verbose .gt. 0) call log(message)
            end if
        end block logging
