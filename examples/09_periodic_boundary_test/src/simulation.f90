      ...
      do while (.not. time%done())
         ...
         do while (time%it .le. time%itmax)
            ...
            scalar_solver: block
               ...
               allocate (SCtmp(cfg%imino_:cfg%imaxo_, cfg%jmino_:cfg%jmaxo_, cfg%kmino_:cfg%kmaxo_, nspec + 1))
                     ...
                     do j = fc%cfg%jmino_, fc%cfg%jmaxo_
                        do i = fc%cfg%imino_, fc%cfg%imaxo_
                           if (nsc .eq. nspec + 1) then
                              if (SCtmp(i, j, k, nsc) .le. 300.0_WP .or. SCtmp(i, j, k, nsc) .ge. 5000.0_WP .and. flag(i, j, k) .eqv. .true.) then
                                 flag(i, j, k) = .true.
                              else
                                 flag(i, j, k) = .false.
                              end if
