...
contains
  ...
  subroutine simulation_init
    ...
    create_solver: block
      use ils_class, only: amg, pcg_amg, pcg_parasail, gmres, gmres_pilut, smg, pfmg, pcg, pcg_pfmg, bicgstab_amg, gmres_amg
    ...
    create_scalar: block
      use ils_class, only: gmres, pcg_amg, pcg_pfmg, gmres_amg, gmres_pilut
    ...
    create_pmesh: block
      integer :: i
      pmesh = partmesh(nvar=5, name='pdf')
      pmesh%varname(1) = 'pT'
      pmesh%varname(2) = 'pO2'
  ...
  subroutine simulation_run
    ...
    do while (.not. time%done())
      ...
      pdf_injection: block
        ...
        call fs%get_bcond('pilot_inj', mybc)
        call pd%inject(dt=time%dt, itr=mybc%itr, U=fs%U, V=fs%V, W=fs%W, Tin=Tin_pi, Hin=Hin_pi, Yin=Yin_pi, svin=svin_pi)
        ! call pd%inject(dt=time%dt, itr=mybc%itr, U=fs%U, V=fs%V, W=fs%W, Tin=Tin_jet, Hin=Hin_jet, Yin=Yin_jet, svin=svin_jet)
        call pd%sync()
