...
module simulation
   ...
   integer, dimension(:, :, :, :), allocatable :: flag_out
...
contains
   ...
   subroutine simulation_init
      ...
      allocate_work_arrays: block
         ...
         allocate (flag_out(cfg%imino_:cfg%imaxo_, cfg%jmino_:cfg%jmaxo_, cfg%kmino_:cfg%kmaxo_, fc%nscalar))
      ...
      initialize_fc: block
         ...
         real(WP) :: moles_fuel
         real(WP) :: T_init, radius, drop_diameter, phi, tmp1
         ...
         call param_read("Drop diameter", drop_diameter)
         ...
         do k = fc%cfg%kmino_, fc%cfg%kmaxo_
            do j = fc%cfg%jmino_, fc%cfg%jmaxo_
               do i = fc%cfg%imino_, fc%cfg%imaxo_
                  ...
                  radius = norm2([fc%cfg%xm(i), fc%cfg%ym(j)] - [0.0_WP, 0.0_WP])

                  if (radius .lt. 0.5_WP*drop_diameter) then
                  ...
                  fc%SC(i, j, k, isc_fuel) = (W_sp(isc_fuel)*tmp1*moles_fuel)/ &
                                             (W_sp(isc_o2) + 3.76_WP*W_sp(isc_n2) + &
                                              (W_sp(isc_fuel)*tmp1*moles_fuel))
      ...
      create_ensight: block
         ...
         call ens_out%add_scalar('T_unbounded', flag_out(:, :, :, nspec + 1))
         call ens_out%add_scalar('Fuel_unbounded', flag_out(:, :, :, sNXC12H26))
         call ens_out%add_scalar('H2O_unbounded', flag_out(:, :, :, sH2O))
