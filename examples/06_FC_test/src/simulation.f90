  ...
  !> Function that localizes the right domain boundary
      ...
      do k=sc%cfg%kmino_,sc%cfg%kmaxo_
         do j=sc%cfg%jmino_,sc%cfg%jmaxo_
            do i=sc%cfg%imino_,sc%cfg%imaxo_
               ...
               fc%rho(i,j,k)=rho0*rho1/((1.0_WP-Z)*rho1+Z*rho0)
            end do
         end do
      end do
   ...
   subroutine ignition_doubledelta()
      ...
      do k=kmin,kmax
         do j=jmin,jmax
            do i=imin,imax
               tmp_sc(i,j,k) = (tmp_sc(i,j,k) - tmp_sc_min) / (tmp_sc_max - tmp_sc_min) * 0.6_WP + 0.7_WP
            end do
         end do
      end do
   ...
   subroutine simulation_init
      ...
      create_scalar: block
         ...
         ! Outflow on the right
         call sc%add_bcond(name='xm_outflow',type=neumann,locator=xm_locator,dir='-x')
         call sc%add_bcond(name='xp_outflow',type=neumann,locator=xp_locator,dir='+x')
         call sc%add_bcond(name='ym_outflow',type=neumann,locator=ym_locator,dir='-y')
         call sc%add_bcond(name='yp_outflow',type=neumann,locator=yp_locator,dir='+y')
      ...
      allocate_work_arrays: block
         ...
         allocate(resSC(sc%cfg%imino_:sc%cfg%imaxo_,sc%cfg%jmino_:sc%cfg%jmaxo_,sc%cfg%kmino_:sc%cfg%kmaxo_, fc%nscalar))
      ...
      initialize_scalar: block
         ...
         do i=sc%cfg%imin,sc%cfg%imax
            if (sc%cfg%xm(i).gt.sc%cfg%x(sc%cfg%imin)+L_buffer) then
               imin=i
               exit
            end if
         end do
         ...
         do i=sc%cfg%imax,sc%cfg%imin,-1
            if (sc%cfg%xm(i).lt.sc%cfg%x(sc%cfg%imax+1)-L_buffer) then
               imax=i
               exit
            end if
         end do
         ...
         do j=sc%cfg%jmin,sc%cfg%jmax
            if (sc%cfg%ym(j).gt.sc%cfg%y(sc%cfg%jmin)+L_buffer) then
            jmin=j
            exit
            end if
         end do
         ...
         do j=sc%cfg%jmax,sc%cfg%jmin,-1
            if (sc%cfg%ym(j).lt.sc%cfg%y(sc%cfg%jmax+1)-L_buffer) then
               jmax=j
               exit
            end if
         end do
         ...
         do k=sc%cfg%kmin,sc%cfg%kmax
            if (sc%cfg%zm(k).gt.sc%cfg%z(sc%cfg%kmin)+L_buffer) then
            kmin=k
            exit
            end if
         end do
         ...
         do k=sc%cfg%kmax,sc%cfg%kmin,-1
            if (sc%cfg%zm(k).lt.sc%cfg%z(sc%cfg%kmax+1)-L_buffer) then
               kmax=k
               exit
            end if
         end do
         ...
         if (sc%cfg%amRoot) call ignition_doubledelta()
         ...
         !    do j=sc%cfg%jmino_,sc%cfg%jmaxo_
         !       do i=sc%cfg%imino_,sc%cfg%imaxo_
         !          sc%SC(i,j,k) = (sc%SC(i,j,k) - tmp_sc_min) / (tmp_sc_max - tmp_sc_min) * 0.6_WP + 0.7_WP
         !       end do
         !    end do
         ! end do

         print*, minval(sc%SC), maxval(sc%SC)
         ! Compute density
         call get_rho()
      ...
      initialize_fc: block
         ...
         do i=1, nspec
            if (fc%SCname(i).eq.fuel) then
               isc_fuel = i
               print *, "Fuel: ", fc%SCname(i), isc_fuel
         ...
         do k=fc%cfg%kmino_,fc%cfg%kmaxo_
            do j=fc%cfg%jmino_,fc%cfg%jmaxo_
               do i=fc%cfg%imino_,fc%cfg%imaxo_
                  ...
                  fc%SC(i,j,k,isc_fuel) = (W_sp(isc_fuel)*sc%SC(i,j,k)*moles_fuel)/&
                     (W_sp(isc_o2)+3.76_WP*W_sp(isc_n2)+&
                     (W_sp(isc_fuel)*sc%SC(i,j,k)*moles_fuel))          
      ...
      initialize_velocity: block
         ...
         !          if (i.ge.imin.and.i.le.imax.and.j.ge.imin.and.j.le.imax.and.k.ge.kmin.and.k.le.kmax) then
         !             fs%U(i,j,k)=0.7_WP * random_normal(m=0.0_WP,sd=0.5_WP)
         !             ! fs%V(i,j,k)=0.7_WP * random_normal(m=0.0_WP,sd=0.5_WP)
      ...
      create_ensight: block
         ...
         call ens_out%add_scalar('YFuel',fc%SC(:,:,:,isc_fuel))
